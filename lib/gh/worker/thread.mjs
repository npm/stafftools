...
const makeRun =
  ...
  (
    command,
    args = [],
    { status: getStatus = property('status'), ...options } = {}
  ) => {
    ...
    const {
      ...
      ? runDryCommand({ debug })
      : spawnSync(command, args, { encoding: 'utf-8', shell: true, ...options })
...
const runCommands = (commands, state, runCommand, { isNested, ...options }) => {
  for (const getCommand of commands) {
    ...
    if (
      ...
      runCommands(command, state, runCommand, { ...options, isNested: true })
    } else {
      ...
      state.result = runCommand(cmd, args, { ...options, ...cmdOptions })
    }
  }
}
